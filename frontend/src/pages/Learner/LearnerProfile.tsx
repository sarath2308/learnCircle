...
export default function LearnerProfile() {
  ...
  // Validation
  const isEmailSame = useMemo(
    () => formEmail.toLowerCase() === userData?.email?.toLowerCase(),
  ...
  if (isLoading)
    return (
      <div className="h-screen flex items-center justify-center bg-white dark:bg-[#0B0F1A]">
        <div className="animate-pulse text-indigo-600 dark:text-indigo-400 font-black tracking-tighter uppercase">
          Initializing Core...
        </div>
      </div>
    );
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <Card
                key={s.label}
                className="border-none bg-white dark:bg-slate-900/50 shadow-xl shadow-slate-200/60 dark:shadow-none rounded-[2rem] hover:-translate-y-1 transition-all"
              >
                <CardContent className="p-8 flex flex-col items-center text-center gap-4">
                  <div
                    className={`p-4 rounded-2xl bg-${s.color}-500/10 text-${s.color}-600 dark:text-${s.color}-400`}
                  >
          ...
          <Card className="border-none bg-white dark:bg-slate-900/60 shadow-2xl shadow-indigo-100 dark:shadow-none rounded-[2.5rem] p-10">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-6 font-bold flex items-center gap-2 uppercase tracking-tight">
              <ChevronRight className="h-4 w-4 text-indigo-500" /> Advanced Tier: Outperforming 85%
              of peers.
            </p>
          </Card>
        </div>
          ...
          <div className="p-10 bg-slate-900 dark:bg-indigo-600 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group border border-white/10">
            <div className="relative z-10">
              ...
              <p className="text-indigo-100/80 text-sm mb-8 font-medium">
                Push through 3 more modules to unlock the "Architect" badge.
              </p>
        ...
        <div className="fixed inset-0 z-[1000] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-[#0B0F1A] h-full shadow-2xl border-l border-slate-200 dark:border-slate-800 flex flex-col animate-in slide-in-from-right duration-500">
            <div className="p-8 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
              ...
              <button
                onClick={() => setIsModalOpen(false)}
                className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-all dark:text-white"
              >
                <X className="h-6 w-6" />
              </button>
            </div>
            ...
            <div className="p-8 flex-1 overflow-y-auto">
              {activeTab === "profile" && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                  <div className="space-y-3">
                    ...
                    <Input
                      className="h-14 rounded-2xl bg-slate-50 dark:bg-slate-900 border-none px-5 font-bold text-slate-900 dark:text-white ring-1 ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-indigo-500"
                      value={formName}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            disabled={
                              timeLeft > 0 || otpLoading || isEmailSame || !formEmail.includes("@")
                            }
                            onClick={async () => {
                              try {
                        ...
                        {isEmailSame && (
                          <p className="text-[10px] text-amber-600 font-black flex items-center gap-1.5 px-1 uppercase tracking-tighter">
                            <AlertCircle className="h-3 w-3" /> Input must differ from current
                            email.
                          </p>
                        )}
                      </div>
                      ...
                      <div className="p-8 bg-indigo-600/5 dark:bg-indigo-500/5 border border-indigo-600/10 dark:border-indigo-500/20 rounded-[2rem] space-y-6">
                        <div className="text-center space-y-2">
                          ...
                          <p className="text-[10px] text-slate-500 font-bold italic">
                            Check your inbox for the 6-digit key.
                          </p>
                        </div>
              ...
              {activeTab === "password" && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                  <div className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <Input
                        type="password"
                        placeholder="••••••••"
                        className="h-14 bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-800 rounded-2xl px-5 font-bold text-slate-900 dark:text-white"
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={`h-14 bg-slate-50 dark:bg-slate-900 border-none ring-1 rounded-2xl px-5 font-bold text-slate-900 dark:text-white ${isPasswordWeak || isPasswordSame ? "ring-red-500/50" : "ring-slate-200 dark:ring-slate-800"}`}
                        value={passwords.new}
                        onChange={(e) => setPasswords({ ...passwords, new: e.target.value })}
                      />

                      {isPasswordWeak && (
                        <p className="text-[9px] text-red-500 font-black uppercase tracking-tight">
                          Security failure: Key too short.
                        </p>
                      )}
                      {isPasswordSame && (
                        <p className="text-[9px] text-red-500 font-black uppercase tracking-tight">
                          Logic failure: Key must be unique.
                        </p>
                      )}
                    </div>
                  </div>
                  <Button
                    disabled={
                      passwordLoading || isPasswordWeak || isPasswordSame || !passwords.current
                    }
