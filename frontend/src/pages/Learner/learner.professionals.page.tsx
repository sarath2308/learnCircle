        ...
        <section className="lg:col-span-9 space-y-6">
          ...
          {profileData.length > 0 && (
            <div className="flex items-center justify-center gap-1 pt-4">
              ...
              <span className="text-xs font-medium px-4 dark:text-slate-400">
...
const SidebarBox = ({ title, sessionData }: { title: string; sessionData?: SessionBooking[] }) => (
  <div className="p-5 rounded-xl border border-dashed border-slate-300 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50">
    <h2 className="text-xs font-bold text-slate-900 dark:text-white mb-4 uppercase tracking-tighter opacity-70">
      {title}
    </h2>
    {sessionData && sessionData.length > 0 ? (
      ...
      <div className="py-8 text-center text-[10px] text-slate-400 italic font-medium uppercase">
        No Active Sessions
      </div>
    )}
  </div>
