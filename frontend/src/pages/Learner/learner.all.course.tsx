    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          size="sm"
          onClick={resetAll}
          className="text-indigo-600 dark:text-indigo-400 h-8"
        >
        ...
        >
          {["All", "Free", "Paid"].map((t) => (
            <div key={t} className="flex items-center space-x-2">
              ...
              <label
                htmlFor={`price-${t}`}
                className="text-sm font-medium text-slate-600 dark:text-slate-400 capitalize cursor-pointer"
              >
                {t}
              </label>
            </div>
          ...
          {categoryData.map((cat: any) => (
            <div key={cat.id}>
              ...
              >
                {cat.name}
                <ChevronRight
                  className={`w-4 h-4 transition-transform ${filters.categoryId === cat.id ? "rotate-90" : ""}`}
                />
              </button>
    ...
    <div className="bg-white dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="flex flex-col gap-6 mb-8">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button
                    variant="outline"
                    className="lg:hidden flex items-center gap-2 dark:bg-slate-900 dark:border-slate-800"
                  >
                ...
                <SheetContent
                  side="left"
                  className="w-[300px] bg-white dark:bg-slate-950 p-6 overflow-y-auto border-r dark:border-slate-800"
                >
          ...
          {/* Sorting Bar */}
     {/* Sorting Bar */}
<div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6 mb-2">
  <div className="flex flex-wrap items-center gap-3">
    <span className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mr-2">
      Sort Results By:
    </span>
    ...
    <button
      ...
      className={`inline-flex items-center justify-center h-9 px-4 text-xs font-bold gap-2 rounded-xl transition-all duration-300 border-2 
        ${filters.sortDate !== 0 
          ? "border-indigo-600 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-300 shadow-sm shadow-indigo-200/50" 
    ...
    >
      <Calendar size={14} className={filters.sortDate !== 0 ? "text-indigo-600" : "text-slate-400"} />
      Date
      {filters.sortDate !== 0 && (
        <span className="ml-1 bg-indigo-600 text-white w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-black scale-110">
          {filters.sortDate === 1 ? "↓" : "↑"}
        </span>
      )}
    </button>
    ...
    >
      <Star 
        size={14} 
        className={filters.sortRating !== 0 ? "text-indigo-600 fill-indigo-600" : "text-slate-400"} 
      />
      Rating
      {filters.sortRating !== 0 && (
        <span className="ml-1 bg-indigo-600 text-white w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-black scale-110">
          {filters.sortRating === 1 ? "↓" : "↑"}
        </span>
      )}
    </button>
    ...
    >
      <Tag size={14} className={filters.sortPrice !== 0 ? "text-indigo-600" : "text-slate-400"} />
      Price
      {filters.sortPrice !== 0 && (
        <span className="ml-1 bg-indigo-600 text-white w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-black scale-110">
          {filters.sortPrice === 1 ? "↓" : "↑"}
  ...
  <div className="flex items-center bg-slate-100 dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-800">
    <p className="text-xs font-bold text-slate-500 uppercase tracking-tight">
      Showing <span className="text-indigo-600 dark:text-indigo-400 font-black px-1">{allCourses.length}</span> Results
    </p>
  </div>
</div>
        </div>
          ...
          <main className="flex-1">
            {coursesLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-32 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                ...
                <Button onClick={resetAll} variant="link" className="text-indigo-600 mt-2">
                  Clear all filters
                </Button>
              </div>
            )}
