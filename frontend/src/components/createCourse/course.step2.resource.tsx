...
import { 
  Plus, ChevronLeft, ChevronRight, BookOpen, Sparkles, Loader2, AlertTriangle 
} from "lucide-react";
...
const Step2Resources = ({ handleNext, handlePrev }: IStep2Props) => {
  ...
  const onSubmit = async (data: ChapterSchemaType) => {
    ...
    try {
      if (editingChapter) {
        await updateChapter.mutateAsync({ chapterId: editingChapter.id, title: data.title,description: data.description  });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 dark:text-slate-100">
      <div className="flex items-center justify-between border-b pb-6 dark:border-slate-800">
        ...
        <Button 
          onClick={() => { setEditingChapter(null); setModalOpen(true); }}
      ...
      <div className="min-h-[300px]">
        {chapterData.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-3xl border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20">
            ...
            <Button variant="outline" onClick={() => setModalOpen(true)} className="mt-4 rounded-xl font-bold">Add First Chapter</Button>
          </div>
        ) : (
      ...
      <div className="flex items-center justify-between pt-10 border-t dark:border-slate-800">
        ...
        <Button onClick={handleNext} disabled={chapterData.length === 0} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-10 rounded-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-2">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input {...register("title")} className="h-12 rounded-xl dark:bg-slate-800 dark:border-slate-700" />
            ...
            <div className="space-y-2">
              ...
              <Textarea {...register("description")} rows={4} className="rounded-xl dark:bg-slate-800 dark:border-slate-700" />
              {errors.description && <p className="text-xs font-bold text-red-500">{errors.description.message}</p>}
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t dark:border-slate-800">
            <Button type="button" variant="ghost" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" disabled={isSubmitting} className="bg-blue-600 text-white font-black px-6 rounded-xl">
      ...
      >
        <div className="flex flex-col items-center text-center p-4">
          ...
          <p className="text-slate-500 text-sm mb-6">
            This will permanently delete the chapter and all associated lessons. This action cannot be undone.
          </p>
          <div className="flex w-full gap-3">
            <Button variant="outline" onClick={() => setDeleteModalOpen(false)} className="flex-1 rounded-xl">Cancel</Button>
            <Button variant="destructive" onClick={confirmDelete} className="flex-1 rounded-xl font-bold bg-red-600">
