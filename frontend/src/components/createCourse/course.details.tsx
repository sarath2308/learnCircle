...
const CourseDetails = ({ handleNext }: CourseDetailsProps) => {
  ...
  const { data: subCategoryData, isLoading: subLoading } = useGetSubCategories(courseDetails.category);
  ...
  const [errors, setErrors] = useState<Step1Errors>({
    title: "", description: "", category: "", subCategory: "", skillLevel: "", thumbnail: "",
  });
  ...
  const validate = () => {
    ...
    const newErrors = { title: "", description: "", category: "", subCategory: "", skillLevel: "", thumbnail: "" };
    let isValid = true;
...
if (courseDetails.skillLevel) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto py-10 px-4">
      <Card className="overflow-hidden border-0 shadow-2xl dark:bg-gray-950 dark:border dark:border-gray-800">
        ...
        <CardContent className="p-8 space-y-10">
          ...
          <div className="space-y-4">
            ...
            />
            {errors.title && <p className="text-red-500 text-xs flex items-center gap-1 font-medium"><AlertCircle size={14}/>{errors.title}</p>}
          </div>
          ...
          <div className="space-y-4">
            ...
            />
            {errors.description && <p className="text-red-500 text-xs flex items-center gap-1 font-medium"><AlertCircle size={14}/>{errors.description}</p>}
          </div>
              ...
              >
                ...
                <SelectContent className="bg-white dark:bg-gray-950 border-slate-200 dark:border-gray-800 shadow-2xl z-[100]">
                  {categoryData?.map((cat) => (
                    <SelectItem key={cat.id} value={cat.id} className="py-2.5 cursor-pointer">{cat.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {errors.category && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.category}</p>}
            </div>

            {/* Sub-Category */}
            <div className="space-y-3">
              <Label className="text-sm font-bold text-slate-600 dark:text-slate-400">Sub-Category</Label>
              <Select 
              ...
              >
                <SelectTrigger className={`h-12 bg-white dark:bg-gray-900 border-slate-200 dark:border-gray-800 rounded-xl shadow-sm ${!subCategories.length && 'opacity-50'}`}>
                  <SelectValue placeholder={subLoading ? "Loading..." : (subCategories.length ? "Select" : "No Sub-Categories")} />
                </SelectTrigger>
                <SelectContent className="bg-white dark:bg-gray-950 border-slate-200 dark:border-gray-800 shadow-2xl z-[100]">
                  {subCategories.map((sub) => (
                    <SelectItem key={sub.id} value={sub.id} className="py-2.5 cursor-pointer">{sub.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {errors.subCategory && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.subCategory}</p>}
            </div>

            {/* Skill Level */}
            <div className="space-y-3">
              <Label className="text-sm font-bold text-slate-600 dark:text-slate-400">Target Level</Label>
              <Select 
              ...
              >
                ...
                <SelectContent className="bg-white dark:bg-gray-950 border-slate-200 dark:border-gray-800 shadow-2xl z-[100]">
                  {skillLevels.map((lvl) => (
                    <SelectItem key={lvl.value} value={lvl.value} className="py-2.5 cursor-pointer">{lvl.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {errors.skillLevel && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.skillLevel}</p>}
            </div>
          </div>

          {/* Thumbnail Preview Area */}
          <div className="space-y-4">
            <Label className="text-sm font-bold text-slate-700 dark:text-slate-300">Course Cover Image</Label>
            <AnimatePresence mode="wait">
              {!thumbnailPreview ? (
                ...
                >
                  ...
                  <p className="text-sm font-bold text-slate-900 dark:text-white">Upload thumbnail</p>
                  <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                </motion.label>
              ) : (
                <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative h-72 rounded-2xl overflow-hidden shadow-lg group">
                  <img src={thumbnailPreview} className="w-full h-full object-cover" alt="Preview" />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <Button variant="destructive" size="sm" onClick={() => {setThumbnailPreview(""); setThumbnailFile(null); dispatch(setCourseDetails({ thumbnailUrl: "" }))}} className="rounded-full px-6 font-bold">
            ...
            {errors.thumbnail && <p className="text-red-500 text-xs font-medium italic mt-2">{errors.thumbnail}</p>}
          </div>
        </CardContent>
      </Card>

      <div className="mt-10 flex items-center justify-between">
         <p className="text-sm font-medium text-slate-400 hidden sm:block">Step 1 of 3 • <span className="text-indigo-500 font-bold">Foundation</span></p>
         <Button 
        ...
        >
          {isSubmitting ? <Loader2 className="animate-spin mr-2" /> : (isEdit ? "Update & Continue" : "Save & Continue")}
