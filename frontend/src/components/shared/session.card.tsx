...
export const SessionCard = ({
  ...
  const submitRating = () => {
    ...
    if (handleRating) handleRating(id, ratingData);
      ...
      <Card className="group overflow-hidden border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-950 hover:shadow-xl transition-all duration-500">
        <CardContent className="p-5 flex flex-wrap items-center gap-6">
          ...
          <div className="flex flex-col items-center justify-center bg-indigo-50/50 dark:bg-indigo-950/20 rounded-[1.25rem] p-2 w-16 h-16 shrink-0 border border-indigo-100 dark:border-indigo-900/20 shadow-sm">
            <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase leading-none tracking-tighter">{month}</span>
            <span className="text-2xl font-black text-slate-900 dark:text-slate-50">{day}</span>
          </div>

          <div className="flex-1 min-w-[180px] space-y-2">
            <h3 className="font-black text-slate-900 dark:text-slate-50 text-base leading-tight capitalize truncate tracking-tight">{typeOfSession}</h3>
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className={cn(
                "px-3 py-0.5 text-[9px] uppercase tracking-[0.15em] font-black border-transparent rounded-lg",
                status === BookingStatus.CONFIRM ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40" : "bg-slate-100 text-slate-600"
              )}>
                <span className={cn("mr-2 h-1.5 w-1.5 rounded-full", status === BookingStatus.CONFIRM ? "bg-emerald-500 animate-pulse" : "bg-slate-400")} />
                {status}
              </Badge>
          ...
          <div className="flex flex-wrap items-center gap-5 ml-auto w-full sm:w-auto justify-between sm:justify-end">
            {varient === "learner" && status === BookingStatus.COMPLETED && (
              <div className="flex flex-col items-start sm:items-end gap-1.5 pr-5 sm:border-r border-slate-100 dark:border-slate-800">
                <span className="text-[9px] font-black text-indigo-600 uppercase tracking-[0.2em] flex items-center gap-1.5"><CheckCircle size={10} /> Feedback</span>
                <Button size="sm" className="h-9 text-[10px] font-black bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-5 shadow-lg shadow-indigo-600/10 active:scale-95 transition-all" onClick={() => setIsRatingOpen(true)}>
            ...
            {varient === "professional" && hasTimePassed && status !== BookingStatus.COMPLETED && (
              <div className="flex flex-col items-start sm:items-end gap-1.5 pr-5 sm:border-r border-slate-100 dark:border-slate-800">
                <span className="text-[9px] font-black text-amber-600 uppercase tracking-[0.2em] flex items-center gap-1.5"><HelpCircle size={10} /> Finalize</span>
                <Button size="sm" variant="outline" disabled={isCompleting} className="h-9 text-[10px] font-black border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-600 hover:text-white rounded-xl px-5" onClick={() => handleCompletion?.(id)}>
                  {isCompleting ? <Loader2 size={12} className="animate-spin" /> : <><Check size={12} className="mr-2" /> MARK AS DONE</>}
                </Button>
              </div>
            )}
            ...
            <div className="flex flex-col items-start sm:items-end justify-center pr-5 sm:border-r border-slate-100 dark:border-slate-800 h-10">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.25em]">TIMELINE</span>
            ...
            <div className="shrink-0">
              {status !== BookingStatus.COMPLETED ? (
                <Button onClick={handleJoinClick} className="bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-widest rounded-xl px-8 h-11 shadow-xl shadow-indigo-600/20 active:scale-95 transition-all">
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star
                      ...
                      className={cn(
                        "transition-colors",
                        (hover || rating) >= star ? "fill-indigo-500 text-indigo-500" : "fill-slate-100 text-slate-200 dark:fill-slate-800"
                      )}
                    />
                  </button>
                ))}
