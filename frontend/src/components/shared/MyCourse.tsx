  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 min-h-screen bg-slate-50/30 dark:bg-transparent">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white">My Courses</h1>
        ...
        <div className="flex items-center gap-4 pb-4 w-full md:w-auto">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="icon" className="rounded-xl relative border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <DropdownMenuLabel className="p-0 text-sm font-black uppercase tracking-wider text-slate-900 dark:text-slate-100">Filters</DropdownMenuLabel>
                {activeFilterCount > 0 && (
                  <button onClick={resetFilters} className="text-[11px] font-bold text-blue-600 hover:underline">Reset</button>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500">Category</Label>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-white dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 text-center">
          ...
          <Button variant="link" onClick={resetFilters} className="mt-2 text-blue-600 font-bold">Clear all filters</Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {data.courseData.map((course: any) => (
            <CourseCard key={course.id} course={course} onEdit={onEdit} onPublish={onPublish} onView={onView} variant="creator" />
          ))}
        </div>
      )}
    </div>
...
function FilterField({ label, value, onValueChange, options }: any) {
  return (
    <div className="space-y-1.5">
      <Label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-tight">{label}</Label>
      <Select value={value} onValueChange={onValueChange}>
        ...
        <SelectContent className="bg-white dark:bg-[#1C2331] border-slate-200 dark:border-slate-800">
          <SelectItem value="all" className="text-xs">All {label}s</SelectItem>
