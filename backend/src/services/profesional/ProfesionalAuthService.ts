...
import { IBaseRepo } from "../../Repositories/Base/base";

export class ProfesionalAuthService implements IAuthService<IProfessional> {
  constructor(
  ...
  async login(email: string, password: string) {
    try {
      ...
      if (match) {
        ...
        const jwt = await this.accesToken.signAccessToken({ id: match.id, role: match.role });
  ...
  async resendOtp(email: string, type: string) {
    try {
      if (type === "forgot") {
        ...
        await this.redis.set(`signup:${email}`, JSON.stringify({ ...parsed, otp }), 60);
