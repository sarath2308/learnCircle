...
export class ProfessionalProfileController implements IProfessionalProfileController {
  ...
  async uploadProfile(req: IAuthRequest, res: Response): Promise<void> {
    if (!req.user) {
  ...
  async updateProfileImage(req: IAuthRequest, res: Response): Promise<void> {
  ...
  async reqEmailOtp(req: IAuthRequest, res: Response): Promise<void> {
    const userId = req.user?.userId as string;
    await this._profileService.requestChangeEmail(userId, req.body.newEmail);
    res.status(HttpStatus.OK).json({ success: true });
  }
  async verifyAndUpdateEmail(req: IAuthRequest, res: Response): Promise<void> {
    const userId = req.user?.userId as string;
